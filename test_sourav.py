...
def load_ckpt_updated(model, checkpoint_path, prev_classes, current_classes, eval=False):
    """Updated load_ckpt to handle both old and new checkpoint formats"""
    checkpoint = torch.load(checkpoint_path)
    
    # Check if checkpoint is in new format (with model_state_dict wrapper)
    if 'model_state_dict' in checkpoint:
        state_dict = checkpoint['model_state_dict']
        print(f"✓ Loading checkpoint in NEW format (with optimizer state)")
    else:
        # Old format - direct state_dict
        state_dict = checkpoint
        print(f"✓ Loading checkpoint in OLD format (direct state_dict)")
    ...
    if eval:
        ...
        if state_dict.get('frozen_embeddings') is not None:
            model.frozen_embeddings = torch.nn.Parameter(state_dict['frozen_embeddings'])
        else:
            model.frozen_embeddings = None
        if state_dict.get('embeddings') is not None:
            model.embeddings = torch.nn.Parameter(state_dict['embeddings'])
        else:
    ...
    if prev_classes == 0:
        ...
        model.frozen_embeddings = torch.nn.Parameter(freeze)
        model.embeddings = None
        return model
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        model = load_ckpt_updated(model, args.ckpt,cfg.TEST.PREV_INTRODUCED_CLS,cfg.TEST.CUR_INTRODUCED_CLS,eval=True)
        model = model.cuda()
